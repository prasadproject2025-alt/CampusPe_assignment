...
const classifiers: Array<{ field: string; patterns: RegExp[] }> = [
  ...
  { field: 'portfolio_url', patterns: [/\bportfolio\b(?! password)/, /^website$/, /\bpersonal website\b/] },
  ...
  { field: 'location', patterns: [/\bcurrent location\b/, /\bwhere are you (?:currently )?(?:based|located)\b/, /^location(?:\s+city)?$/, /^address$/] },
