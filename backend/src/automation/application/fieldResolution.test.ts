...
import { isEmployerSpamRejection, parseSubmissionFailureCode } from './fieldResolution.js'
...
test('detects Ashby spam rejection copy', () => {
  assert.equal(isEmployerSpamRejection('SUBMISSION_FAILED: The employer flagged this submission as possible spam.'), true)
  assert.equal(isEmployerSpamRejection('ANSWER_REQUIRES_USER: Location'), false)
})
